...
class BackupScheduleDeleteCommand extends CommandBase
{

    protected function configure()
    {
        $this->setName('backup:schedule:delete');
        $this->setDescription('Delete a scheduled backup policy');
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$questionHelper->confirm('Are you sure you want to delete this backup policy?')) {
            return 1;
        }
    ...
    {
        $this->stdErr->writeln('');
        $this->runOtherCommand('backup:schedule:list', [

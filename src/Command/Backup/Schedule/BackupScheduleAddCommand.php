...
class BackupScheduleAddCommand extends CommandBase
{
    ...
    {
        $this->setName('backup:schedule:add')
            ->setDescription('Add a scheduled backup policy');
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->stdErr->writeln(sprintf(
            'Created scheduled backup policy: interval <info>%s</info>, count <info>%d</info>.',
            $values['interval'],
            $values['count']
        ));
    ...
    private function getFields()
    {
        return [
            'interval' => new Field('Interval', [
                'description' => 'The time interval between scheduled backups',
                'default' => '1h',
            ...
            'count' => new Field('Count', [
                'description' => 'The maximum number of backups to keep for this policy',

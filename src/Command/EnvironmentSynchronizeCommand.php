...
class EnvironmentSynchronizeCommand extends EnvironmentCommand
{
    ...
    {
        $this->setName('environment:synchronize')->setDescription('Synchronize an environment.')->addArgument(
            'synchronize',
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$this->operationAllowed('synchronize')) {
            $output->writeln(
              "<error>Operation not permitted: The environment '$environmentId' can't be synchronized.</error>"
            );
        ...
        if ($synchronize = $input->getArgument('synchronize')) {
            ...
            if (!$this->confirm(
              "Are you sure you want to synchronize <info>$parentId</info> to <info>$environmentId</info>? [y/N] ",
              $input,
              $output,
              false
            )
            ...
            $syncCode = $this->confirm(
              "Synchronize code from <info>$parentId</info> to <info>$environmentId</info>? [y/N] ",
              $input,
              $output,
              false
            );
            $syncData = $this->confirm(
              "Synchronize data from <info>$parentId</info> to <info>$environmentId</info>? [y/N] ",
              $input,
              $output,
              false
            );
